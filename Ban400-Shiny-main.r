...
ui <- fluidPage(theme = shinytheme("cosmo"),
                ...
                navbarPage("Portfolio Optimizer",
                           tabPanel("Home"),
                           tabPanel("Code"),
                           windowTitle = "Portfolio Optimizer"),
                
                #  includeCSS("Ban400-theme.css"),
                
                
                
                headerPanel('Find your optimal portfolio'),
                sidebarPanel(
...
server <- function(input, output) {
  ...
  # eventReactive means that it updates the function inputs only when user clicks the "update" button
  ...
  # -- Making output functions to be able to use subsets from the stock_opt_vol, stock_opt_sharpe and sortino_output functions.
  ...
  output$vopt_vol <- renderPrint({
    vol_output()[[1]] # Here we output a subset of vol_output
  })
